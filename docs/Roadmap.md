...
| v3.0 Phase 3D | CLOSED — GO | Transaction Import Confirm 后端能力 | [Phase 3D Closing Review](review/V3.0-Phase3D-Transaction-Import-Confirm-Closing-Review.md) |
...
- `TRANSFER` / `REFUND`；
- Transaction Import Frontend（前端 `NOT STARTED`）；
- 银行或券商账单导入；
- 历史持仓与收益曲线；
- 对账、去重和余额校准；
- PWA 与移动端快速录入。
...
中期重点是在已关闭的可理解、可审计读取体验上降低手工数据录入成本。普通 Transaction 的 CSV / XLSX Import Contract 已冻结字段映射、重复检测、幂等、原子性与错误恢复规则；Phase 3D Confirm 后端已 `CLOSED — GO`，下一阶段为 Transaction Import Frontend。银行或券商账单导入仍是未冻结候选。
