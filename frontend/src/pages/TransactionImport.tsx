...
export default function TransactionImport() {
    ...
    const step = coordinator.readyForConfirm ? 'READY_FOR_CONFIRM' : coordinator.preview?.sessionStatus === 'PREVIEW_READY' && !coordinator.mappingMode ? 'WARNING_REVIEW' : coordinator.preview ? 'MAPPING' : 'UPLOAD';
    return <div className="import-workspace">
        ...
        {coordinator.preview?.sessionStatus === 'PREVIEW_READY' && !coordinator.mappingMode && <ImportPreviewPanel preview={coordinator.preview} rows={coordinator.visibleRows} page={coordinator.page} pageLast={coordinator.pageLast} busy={coordinator.busy} workflow={coordinator.workflow} warnings={coordinator.warnings} acknowledged={coordinator.acknowledged} hydrationError={coordinator.hydrationError} readyForConfirm={coordinator.readyForConfirm} onPage={coordinator.loadVisiblePage} onBackToMapping={coordinator.backToMapping} onRetryHydration={coordinator.retryHydration} onToggleWarningGroup={coordinator.toggleWarningGroup} onRepreview={coordinator.repreview} />}
    </div>;
}
