...
export default function Dashboard() {
    ...
    if (error) return <EmptyState message={error} />; if (!data) return <EmptyState message="加载中…" />;
    return <div>
        <header className="page-header"><div><h1 className="page-header__title">财务概览</h1><p className="page-header__subtitle">基于当前账户、资产和流水数据的实时汇总</p></div></header>
        ...
        <section className="content-card"><h2 className="content-card__title"><span className="content-card__title-icon"><Icon name="transactions" size={17} /></span>最近交易</h2><div className="table-scroll"><table className="data-table"><thead><tr><th>日期</th><th>账户</th><th>分类</th><th>类型</th><th className="cell-number">金额</th></tr></thead><tbody>{data.recentTransactions.map(tx => <tr key={tx.id}><td>{tx.date}</td><td>{tx.account}</td><td>{tx.category}</td><td>{formatTransactionType(tx.type)}</td><td className="cell-number"><span className={`amount amount--${tx.type === 'INCOME' ? 'positive' : tx.type === 'EXPENSE' ? 'negative' : 'neutral'}`} style={{ color: transactionAmountColor(tx.type) }}>{formatTransactionAmount(tx.type, tx.amount)}</span></td></tr>)}{data.recentTransactions.length === 0 && <tr><td colSpan={5}><EmptyState message="暂无最近流水" /></td></tr>}</tbody></table></div></section>
    </div>;
}
